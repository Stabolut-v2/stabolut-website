# Fee Structure & Treasury Allocation
...
This page explains fees, spread capture, treasury allocation, and sustainability. The discussion
...
Protocol mechanics describe the accounting layer behind the user interface. Stabolut coordinates
deposits, mints, redemptions, reserve reporting, yield routing, fee capture, and treasury
allocations through a vault-centered architecture.

The same principles appear repeatedly: self-custody for users, on-chain proof of reserves for
observers, Aave v3 integration for yield, and a multi-currency architecture for future
expansion. Each mechanism is valuable on its own, but the system is designed to work as a
complete stablecoin standard.

In practical terms, mechanics are what let USB behave like a stablecoin and sUSB behave like a
yield-bearing stablecoin without hiding the backing model behind off-chain promises.
...
Fee Structure & Treasury Allocation should leave the reader with the same durable conclusion:
